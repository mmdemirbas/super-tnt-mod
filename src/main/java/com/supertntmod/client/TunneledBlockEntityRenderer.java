 ...
 * Biyom tonlaması (çimen yeşili vb.) tintIndex ile doğru şekilde uygulanır.
 */
public class TunneledBlockEntityRenderer implements BlockEntityRenderer<TunneledBlockEntity, TunneledBlockEntityRenderer.TunneledRenderState> {
    ...
    public void updateRenderState(TunneledBlockEntity entity, TunneledRenderState state, float tickDelta,
        ...
        BlockStateModel model = client.getBlockRenderManager().getModel(state.originalBlockState);
        ...
        for (Direction dir : Direction.values()) {
            int i = dir.ordinal();
            state.sprites[i] = fallback;
            state.tintColors[i] = -1;
            for (BlockModelPart part : parts) {
                List<BakedQuad> quads = part.getQuads(dir);
                if (!quads.isEmpty()) {
                    BakedQuad quad = quads.get(0);
                    state.sprites[i] = quad.sprite();
                    int tintIndex = quad.tintIndex();
                    if (tintIndex >= 0) {
                        state.tintColors[i] = client.getBlockColors().getColor(
                                state.originalBlockState, world, entity.getPos(), tintIndex);
                    }
                    break;
                }
            }
    ...
    public void render(TunneledRenderState state, MatrixStack matrices,
        ...
        if (state.originalBlockState == null || state.sprites[0] == null) return;
        ...
        Sprite[] sprites = state.sprites;
        int[] tintColors = state.tintColors;
                ...
                for (int y = 0; y < 4; y++) {
                    for (int x = 0; x < 4; x++) {
                        ...
                        if (!isFilledAt(subVoxels, x, y - 1, z)) {
                            ...
                            int c = tintColors[Direction.DOWN.ordinal()];
                        ...
                        if (!isFilledAt(subVoxels, x, y + 1, z)) {
                            ...
                            int c = tintColors[Direction.UP.ordinal()];
                        ...
                        if (!isFilledAt(subVoxels, x, y, z - 1)) {
                            ...
                            int c = tintColors[Direction.NORTH.ordinal()];
                        ...
                        if (!isFilledAt(subVoxels, x, y, z + 1)) {
                            ...
                            int c = tintColors[Direction.SOUTH.ordinal()];
                        ...
                        if (!isFilledAt(subVoxels, x - 1, y, z)) {
                            ...
                            int c = tintColors[Direction.WEST.ordinal()];
                        ...
                        if (!isFilledAt(subVoxels, x + 1, y, z)) {
                            ...
                            int c = tintColors[Direction.EAST.ordinal()];
    ...
    public static class TunneledRenderState extends BlockEntityRenderState {
        ...
        public final Sprite[] sprites = new Sprite[Direction.values().length];
        public final int[] tintColors = new int[Direction.values().length];
    }
}

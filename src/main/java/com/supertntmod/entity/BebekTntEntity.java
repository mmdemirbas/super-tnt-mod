...
/**
 * 👶 Bebek TNT — patladığında etrafa "Bebek" isimli yavru köylüler saçar. Pembe temalı.
 */
public class BebekTntEntity extends TntEntity {
    private static final int VILLAGER_COUNT = 15;
    private static final double SPREAD = 6.0;
    private boolean done = false;

    public BebekTntEntity(EntityType<? extends TntEntity> type, World world) {
        super(type, world);
        this.setFuse(80);
    }

    public BebekTntEntity(World world, double x, double y, double z, @Nullable LivingEntity igniter) {
        super(ModEntities.BEBEK_TNT, world);
    ...
    public void tick() {
        if (!done && this.getFuse() <= 1 && !this.getEntityWorld().isClient()) {
            ...
            world.playSound(null, cx, cy, cz,
                    SoundEvents.ENTITY_GENERIC_EXPLODE.value(), SoundCategory.BLOCKS, 1.5f, 1.6f);
            ...
            if (world instanceof ServerWorld sw) {
                for (int i = 0; i < VILLAGER_COUNT; i++) {
                    ...
                    villager.setCustomName(Text.literal("Bebek"));

...
/**
 * 👩 Anne TNT — patladığında etrafa "Anne" isimli köylüler saçar. Pembe temalı.
 */
public class AnneTntEntity extends TntEntity {
    private static final int VILLAGER_COUNT = 12;
    private static final double SPREAD = 6.0;
    private boolean done = false;

    public AnneTntEntity(EntityType<? extends TntEntity> type, World world) {
        super(type, world);
        this.setFuse(80);
    }

    public AnneTntEntity(World world, double x, double y, double z, @Nullable LivingEntity igniter) {
        super(ModEntities.ANNE_TNT, world);
        ...
        if (!done && this.getFuse() <= 1 && !this.getEntityWorld().isClient()) {
            ...
            if (world instanceof ServerWorld sw) {
                for (int i = 0; i < VILLAGER_COUNT; i++) {
                    ...
                    VillagerEntity villager = EntityType.VILLAGER.create(sw, net.minecraft.entity.SpawnReason.MOB_SUMMONED);
                    if (villager == null) continue;
                    villager.setPosition(cx + ox, cy + 1, cz + oz);
                    villager.setCustomName(Text.literal("Anne"));
